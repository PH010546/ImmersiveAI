...
1. **Original Mod Author's Code & Prompts (原 Mod 開發者的原始程式碼、架構與 Prompt - Anton / Trax)**:
   - **嚴禁擅自直接改動**。
...
4. **Discussion vs. Implementation Boundary**:
   ...
   - 必須獲得使用者明確同意指令（如「好」、「可以」、「請修改」、「請開始」、「同意」）後，始得執行程式碼修改與檔案寫入。
